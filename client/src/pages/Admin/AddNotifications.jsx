import React, { useState, useEffect } from "react";
import {
    HtmlEditor,
    Image,
    Inject,
    Link,
    QuickToolbar,
    RichTextEditorComponent,
    Toolbar,
} from "@syncfusion/ej2-react-richtexteditor";
import { LuDot } from "react-icons/lu";
import { useFormik } from "formik";
import * as Yup from "yup";
import { useNavigate } from "react-router-dom";
import Skeleton from "react-loading-skeleton";
import { useDispatch } from "react-redux";
import { addItem } from "../../services/globalService";
import { addNewData } from "../../Rtk/slices/addNewSlice";

export default function AddNotifications() {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const [loading, setLoading] = useState(false);
    const [status, setStatus] = useState("idle");

    // Toolbar settings for RichTextEditor
    const toolbarSettings = {
        items: [
            "Bold",
            "Italic",
            "Underline",
            "StrikeThrough",
            "FontName",
            "FontSize",
            "FontColor",
            "BackgroundColor",
            "|",
            "Alignments",
            "OrderedList",
            "UnorderedList",
            "Outdent",
            "Indent",
            "|",
            "CreateLink",
            "Image",
            "SourceCode",
            "Undo",
            "Redo",
        ],
    };

    const routes = ["Notification", "Add Notifications"];

    const formik = useFormik({
        initialValues: {
            name: "",
            content: "",
            documentType: "SMS"
        },
        validationSchema: Yup.object({
            name: Yup.string().required("Subject can't be empty"),
            content: Yup.string().required("Content can't be empty"),
        }),
        onSubmit: async (values, { resetForm }) => {
            try {
                 setStatus("loading");
                await dispatch(
                    addNewData({
                        path: "emailtemplates",
                        itemData: values,
                    })
                );
                 navigate("/notifications");
            } catch (error) {
                console.error(error);
            } finally {
                setStatus("idle");
            }
        },
    });



    return (
        <div className="p-4 md:p-10 bg-white rounded-3xl">
            {/* Header */}
            <div className="flex lg:flex-row flex-col items-center justify-between">
                <div>
                    <h1 className="text-xl font-semibold leading-[1.5] text-[#1C252E] dark:text-white">
                        Notification Template
                    </h1>

                    <ul className="flex flex-row items-center space-x-1 text-sm py-2">
                        {routes.map((route, index) => (
                            <li
                                key={index}
                                className={`flex items-center ${index === routes.length - 1
                                    ? "text-gray-400"
                                    : "text-slate-900 dark:text-gray-200 hover:underline"
                                    }`}
                            >
                                {index > 0 && (
                                    <LuDot className="text-lg text-gray-400 font-bold mx-1" />
                                )}
                                {route}
                            </li>
                        ))}
                    </ul>
                </div>
            </div>

            {/* Form */}
            <div className="my-6">
                {loading ? (
                    <div>
                        <Skeleton width="100%" height="20rem" />
                        <Skeleton width="75%" className="mb-2" />
                        <Skeleton width="50%" className="mb-2" />
                        <Skeleton height="2rem" className="mb-2" />
                    </div>
                ) : (
                    <form
                        className="flex flex-col space-y-6"
                        onSubmit={formik.handleSubmit}
                    >
                        {/* Subject input */}
                        <div>
                            <label
                                htmlFor="title"
                                className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 after:content-['*'] after:text-red-500 after:ml-1"
                            >
                                Subject
                            </label>
                            <input
                                type="text"
                                id="title"
                                name="name"
                                onChange={formik.handleChange}
                                onBlur={formik.handleBlur}
                                value={formik.values.name}
                                placeholder="Enter notification title"
                                className="border border-gray-300 bg-transparent rounded-md p-2 w-full focus:ring-1 focus:ring-slate-800 focus:outline-none bg-[#F6F7F8]"
                            />
                            {formik.touched.name && formik.errors.name && (
                                <div className="text-red-600 italic mt-1 text-[12px]">
                                    {formik.errors.name}
                                </div>
                            )}
                        </div>

                        {/* RichTextEditor */}
                        <div className="w-full">
                            <label
                                htmlFor="content"
                                className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 after:content-['*'] after:text-red-500 after:ml-1"
                            >
                                Content
                            </label>

                            <RichTextEditorComponent
                                id="content"
                                name="content"
                                aria-label="Notification Content"
                                value={formik.values.content}
                                change={(e) => formik.setFieldValue("content", e.value)}
                                toolbarSettings={toolbarSettings}
                                className="border border-dotted border-gray-300 rounded-md bg-[#F6F7F8]"
                            >
                                <Inject
                                    services={[HtmlEditor, Toolbar, Image, Link, QuickToolbar]}
                                />
                            </RichTextEditorComponent>

                            {formik.touched.content && formik.errors.content && (
                                <div className="text-red-600 italic mt-1 text-[12px]">
                                    {formik.errors.content}
                                </div>
                            )}
                        </div>

                        {/* Submit */}
                        <div className="flex gap-2 items-center">
                            <button
                                type="submit"
                                disabled={  formik.isSubmitting}
                                className={`font-thin px-10 max-w-sm mt-4 ${!formik.isValid || status === "loading"
                                    ? "bg-gray-800 text-white cursor-not-allowed"
                                    : "blackbutton"
                                    }`}
                            >
                                {status === "loading" ? (
                                    <>
                                        <svg
                                            aria-hidden="true"
                                            role="status"
                                            className="inline w-4 h-4 me-3 text-white animate-spin"
                                            viewBox="0 0 100 101"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                        >
                                            <path
                                                d="M100 50.5908C100 78.2051 77.6142 100.591 
                        50 100.591C22.3858 100.591 0 78.2051 
                        0 50.5908C0 22.9766 22.3858 0.59082 
                        50 0.59082C77.6142 0.59082 100 22.9766 
                        100 50.5908ZM9.08144 50.5908C9.08144 
                        73.1895 27.4013 91.5094 50 91.5094C72.5987 
                        91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 
                        27.9921 72.5987 9.67226 50 9.67226C27.4013 
                        9.67226 9.08144 27.9921 9.08144 50.5908Z"
                                                fill="#E5E7EB"
                                            />
                                            <path
                                                d="M93.9676 39.0409C96.393 38.4038 
                        97.8624 35.9116 97.0079 33.5539C95.2932 
                        28.8227 92.871 24.3692 89.8167 20.348C85.8452 
                        15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 
                        4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 
                        0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 
                        1.69328 37.813 4.19778 38.4501 6.62326C39.0873 
                        9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 
                        9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 
                        10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 
                        17.9648 79.3347 21.5619 82.5849 25.841C84.9175 
                        28.9121 86.7997 32.2913 88.1811 35.8758C89.083 
                        38.2158 91.5421 39.6781 93.9676 39.0409Z"
                                                fill="currentColor"
                                            />
                                        </svg>
                                        Loading...
                                    </>
                                ) : (
                                    "Add Notification"
                                )}
                            </button>
                        </div>
                    </form>
                )}

                {/* Placeholder section */}
                <div className="flex flex-col mt-8 space-y-3 text-[#969695] bg-slate-700 p-6 rounded-md w-full md:w-[800px] text-sm shadow-lg">
                    <h1 className="text-md font-semibold text-white">Placeholders</h1>
                    <ul className="space-y-2 list-disc list-inside">
                        <li>
                            <strong>#NAME#:</strong> Client name placeholder
                        </li>
                        <li>
                            <strong>#LOGINLINK#:</strong> Login link placeholder
                        </li>
                        <li>
                            <strong>#COMPANYNAME#:</strong> Company name placeholder
                        </li>
                        <li>
                            <strong>#ACCOUNTANTNAME#:</strong> Accountant or RM name placeholder
                        </li>
                        <li>
                            <strong>#SIGNATURE#:</strong> Signature placeholder
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    );
}
