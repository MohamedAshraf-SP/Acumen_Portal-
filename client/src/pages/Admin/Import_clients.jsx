...
import successUploadimg from "/images/load_Client/success.webp";
// import formik & Yup
...
const Import_clients = () => {
  const routes = ["Dashboard", "Import Clients"];
  ...
  //  handle fire api import client from csv file
  const handleUploadCsv = async (csvFile) => {
    try {
      setStatus("loading");

      const formData = new FormData();
      formData.append("clients", csvFile.clients);
      const response = await axios.post(`${api}/helpers/importCSV`, formData, {
  ...
  return (
    <>
      <div className="my-8 py-4 px-4 rounded-lg shadow-sm     h-full overflow-scroll dark:bg-secondary-dark-bg dark:text-gray-200">
        {/* Header */}
        <div className=" p-4  ">
