...
export default function ViewClientCard({ item }) {
  ...
  const handleConmpanyAction = (path, action, companyId) => {
    setSelectedItem({ path, companyId });
    setTooltipVisible(!tooltipVisible);
    if (action == "delete") {
      const result = dispatch(
        setdeleteHintmsg({ show: true, targetId: companyId })
      );
      console.log(result);
    } else if (action === "edit") {
            ...
            {tooltipVisible && (
              <div className="absolute top-2 right-4 min-w-36 bg-white border border-gray-200 rounded-lg shadow-md z-10">
                <ul className="flex flex-col py-1">
                  <li
                    className="px-3 py-2 text-sm text-[#1C252E] hover:bg-[#919eab14] cursor-pointer flex items-center gap-4 transition"
                    onClick={() =>
                      handleConmpanyAction("Companies", "edit", item?._id)
                    }
                  >
                    <MdEdit size={20} />
                    Edit
                  ...
                  <li
                    className="px-3 py-2 text-sm   hover:bg-gray-100 cursor-pointer flex items-center gap-4 transition text-red-500"
                    onClick={() =>
                      handleConmpanyAction("Companies", "delete", item?._id)
                    }
                  >
