  ...
  try {
    ...
    if (email) {
      return res.status(400).json({
        message:
          "Email already associated with an account , Try again with different one.",
        email,
      });
    }
    ...
    const newTask = new TasksDocument({
      ...
      companyName: "default",
      path: req.LEOfile.path,
      department: clientData.department,
      title: "LOE",
    });
    const savedTask = await newTask.save();

    return res.status(201).json({ client: clientData.toJSON() });
  } catch (error) {
    console.log(error);
...
export const getDepartmentClients = async (req, res) => {
  ...
  try {
    const clients = await Client.find({ department: req.body.department })
      .skip(skip)
      .limit(limit);
