...
export const importClientsFromCSV = async (req, res) => {
  try {
    ...
    const csvImportLimit = Math.min(
      results.length,
      process.env.CSV_import_limit || results.length
    );
    ...
    for (let i = 1; i <= csvImportLimit; i++) {
      try {
        ...
        const relatedEntities = await createRelatedEntities(
          company._id,
          results[i]
        );

        // 5. Update the company with related entities
        const updatedCompany = await updateCompanyWithEntities(
          company,
          relatedEntities
        );
