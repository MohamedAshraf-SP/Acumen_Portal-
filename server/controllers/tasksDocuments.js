...
export const getAllTasks = async (req, res) => {
  ...
  try {
    const TasksDocuments = await TasksDocument.find({}).skip(skip).limit(limit); // Skip the specified number of documents.limit(limit);;
    res.status(200).json({
      currentPage: page,
...
export const updateTask = async (req, res) => {
  try {
    ...
    updatedTask = await TasksDocument.findByIdAndUpdate(
      req.params.id,
      updateData,
